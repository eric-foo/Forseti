...
def _community_target_source(tmp_path: Path, ledger: dict) -> tuple[Path, dict]:
    from harness_utils import hash_file
    from runners.run_semantic_evidence_integration import materialize_v3

    artifact_id = "community-native"
    evidence_id = "review-service:capture-1:review-card:09:root"
    native = _artifact(tmp_path, "community_native.json", json.dumps({
        "reviews": [{"text": "The cap split after one week.", "reply": "Preview ends"}]
    }))
    ledger["artifacts"].append({"artifact_id": artifact_id, **native})
    catalog = {
        "schema_version": "product_identity_catalog_v1",
        "products": [{
            "stable_product_id": "summer-fridays-lip-butter-balm",
    ...
    source = {
        ...
        "cycle_id": ledger["cycle_id"],
        "question_id": "phase-a-evidence-integration",
        "question": "What customers report about packaging",
        "corpus_profile": "phase_a_final_acquisition",
        "corpus_scope": "controlled capture with an incomplete reply",
        "corpus_cutoff": "2026-09-05T00:00:00Z",
        "axes": [{"axis_id": "packaging_reliability", "label": "Packaging"}],
        "source_artifacts": [{
            "artifact_id": artifact_id,
            "locator": native["locator"],
            "sha256": hash_file(tmp_path / native["locator"]),
        }],
        "containers": [{
            "container_id": evidence_id,
            "container_type": "conversation",
            "source_artifact_id": artifact_id,
            "captured_leaf_count": 2,
            "source_visible_total": "unavailable",
            "completeness": "partial",
            "captured_at": "2026-09-05T00:00:00Z",
            "capture_boundary": "One visible review; reply truncated; wider pool unavailable.",
        }],
        "captured_items": [{
            "evidence_id": evidence_id,
            "container_id": evidence_id,
            ...
            "source_role": "community_post",
            "source_artifact_id": artifact_id,
            "source_ref": "https://review-service.test/brand#review-card-09",
            "text": "The cap split after one week.",
            "accounting_disposition": "assess",
            "accounting_reason": "Full visible review body in this capture.",
            "product_candidates": ["sf-lbb"],
            "axis_candidates": ["packaging_reliability"],
            "product_context": [{
                "context_type": "thread_title",
                "source_artifact_id": artifact_id,
                "text": "Summer Fridays Lip Butter Balm review",
                "source_ref": "https://review-service.test/brand",
            }],
            ...
            "evidence_id": evidence_id + ":reply",
            "container_id": evidence_id,
            "source_family": "community_review",
            "source_role": "community_post",
            "source_artifact_id": artifact_id,
            "source_ref": "https://review-service.test/brand#review-card-09-reply",
            "text": "Preview ends",
            "accounting_disposition": "blocked",
            "accounting_reason": "Reply preview is truncated.",
        }],
        "product_identity_catalog": catalog,
    }
    input_path = tmp_path / "community_source_input.json"
    input_path.write_text(json.dumps(source), encoding="utf-8")
    source_path = tmp_path / "community_source.json"
    receipt = materialize_v3(
        source_path=input_path, repo_root=tmp_path, source_out=source_path
    )
    assert receipt["status"] == "SEMANTIC_EVIDENCE_SOURCE_V3_MATERIALIZED"
    ledger["artifacts"].append({
        "artifact_id": "community-source",
        "locator": source_path.name,
        "sha256": _artifact_hash(source_path),
    })
    ledger["families"]["community_review"] = {"source_artifact_id": "community-source"}
    ledger["target_reconciliation"][0].update(
        source_family="community_review",
        native_artifact_id=artifact_id,
        evidence_refs=[{"family": "community_review", "unit_id": evidence_id}],
    )
    return source_path, json.loads(source_path.read_text(encoding="utf-8"))


@pytest.mark.parametrize("defect", [
    None, "missing_family", "missing_pointer", "unknown_pointer", "invalid_json",
    "missing_source_hash", "stale_source_hash", "wrong_cycle", "unknown_ref",
    "blocked_ref", "unknown_family", "wrong_role", "unknown_container",
    "crossed_container", "wrong_target_native", "missing_native_ledger",
    "mismatched_native_locator", "changed_native_bytes", "serp_native", "serp_alias",
])
def test_consumer_brand_v3_community_target_uses_verified_source(
    ...
    ledger_path = tmp_path / reference["locator"]
    ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    source_path, source = _community_target_source(tmp_path, ledger)
    target = ledger["target_reconciliation"][0]
    leaf = source["captured_items"][0]
    native = next(row for row in ledger["artifacts"] if row["artifact_id"] == "community-native")
    if defect == "missing_family":
        ledger["families"].pop("community_review")
    elif defect == "missing_pointer":
        ledger["families"]["community_review"] = {}
    elif defect == "unknown_pointer":
        ledger["families"]["community_review"]["source_artifact_id"] = "unknown"
    elif defect == "wrong_cycle":
        source["cycle_id"] = "different-cycle"
    elif defect == "unknown_ref":
        target["evidence_refs"][0]["unit_id"] = "uncaptured-review"
    elif defect == "blocked_ref":
        target["evidence_refs"][0]["unit_id"] += ":reply"
    elif defect == "unknown_family":
        leaf["source_family"] = "invented-community-family"
    elif defect == "wrong_role":
        leaf["source_role"] = "brand_owned"
    elif defect == "unknown_container":
        leaf["container_id"] = "unknown-container"
    elif defect == "crossed_container":
        extra = _artifact(tmp_path, "other_community.json")
        source["source_artifacts"].append({
            "artifact_id": "other-native", "locator": extra["locator"],
            "sha256": hash_file(tmp_path / extra["locator"]),
        })
        source["containers"][0]["source_artifact_id"] = "other-native"
    elif defect == "wrong_target_native":
        target["native_artifact_id"] = "depth-source"
    elif defect == "missing_native_ledger":
        ledger["artifacts"].remove(native)
    elif defect == "mismatched_native_locator":
        native.update(_artifact(tmp_path, "wrong_native.json"))
    elif defect == "changed_native_bytes":
        (tmp_path / native["locator"]).write_text("changed native body", encoding="utf-8")
        native["sha256"] = _artifact_hash(tmp_path / native["locator"])
    elif defect in {"serp_native", "serp_alias"}:
        search_id = "serp-packet"
        search = next(row for row in ledger["artifacts"] if row["artifact_id"] == search_id)
        native.update(locator=search["locator"], sha256=search["sha256"])
        source["source_artifacts"][0].update(
            locator=search["locator"], sha256=hash_file(tmp_path / search["locator"])
        )
        if defect == "serp_native":
            target["native_artifact_id"] = search_id
    source["source_sha256"] = _sha256({k: v for k, v in source.items() if k != "source_sha256"})
    if defect == "missing_source_hash":
        source.pop("source_sha256")
    elif defect == "stale_source_hash":
        source["source_sha256"] = "0" * 64
    source_path.write_text(
        "not JSON" if defect == "invalid_json" else json.dumps(source), encoding="utf-8"
    )
    next(row for row in ledger["artifacts"] if row["artifact_id"] == "community-source")[
        "sha256"
    ] = _artifact_hash(source_path)
    _rewrite_depth_reference(seal, ledger_path, ledger)
    ...
    if defect is None:
        ...
        assert any(
            finding.startswith((
                "missing_community_target_semantic_source",
                "invalid_community_target_semantic_source",
                "unresolved_consumer_target_evidence_ref:target-1",
                "consumer_target_evidence_native_body_mismatch:target-1",
            ))
            for finding in findings
        ), findings
...
def test_consumer_brand_v3_community_target_does_not_add_independent_support(
    tmp_path: Path,
) -> None:
    seal = _blocked_seal(tmp_path)
    reference = _consumer_depth_ledger(tmp_path)
    ledger_path = tmp_path / reference["locator"]
    ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    _community_target_source(tmp_path, ledger)
    community_ref = ledger["target_reconciliation"][0]["evidence_refs"][0]
    ledger["product_axes"][0]["support_refs"] = [
        {"family": "reddit_forum", "unit_id": f"thread-{index}",
         "contribution": "corroborates", "choice": "subject"}
        for index in range(3)
    ] + [{**community_ref, "contribution": "corroborates", "choice": "subject"}]
    _rewrite_depth_reference(seal, ledger_path, ledger)

    findings = _validate(tmp_path, _make_passing(seal))

    assert not any("consumer_target_evidence_ref" in finding for finding in findings)
