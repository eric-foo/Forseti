...
def test_direct_hrefs_skips_google_internal_anchors() -> None:
    ...
    assert all("google.com" not in href for href in href_map.values())
    assert any("byrdie.com" in href for href in href_map.values())
...
def test_query_rewritten_is_true_when_google_rewrites_the_query() -> None:
    record = build_google_serp_content_record(
        ...
        final_url="https://www.google.com/search?q=summer+friday+review",
        requested_url=_QUERY_URL,
    )
...
def test_invariants_accept_gapped_but_ascending_order() -> None:
    """Gaps are normal: the title/source pairing pass drops rows after renumbering."""
    record = {
    ...
    assert check_serp_invariants(unit="u", record=record, minimum_rows=3) == []
...
def test_invariants_reject_non_ascending_order() -> None:
    ...
    assert [v.invariant for v in violations] == ["order_monotonicity"]
