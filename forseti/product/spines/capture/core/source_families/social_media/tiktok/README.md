...
| Live staging | `orca-harness/source_capture/tiktok/live_batch_probe.py`; runner `run_source_capture_tiktok_live_batch_probe.py` | One creator per invocation; validated retained-session profile or logged-out page-owned observation; a profile-bound Chrome CDP route selects one usable loopback endpoint owned by a root process launched with the configured retained profile; sanitized staging by default. |
