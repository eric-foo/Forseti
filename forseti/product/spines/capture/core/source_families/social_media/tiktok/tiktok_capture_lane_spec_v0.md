...
derived_from:
  - forseti/product/spines/capture/core/source_families/social_media/tiktok/tiktok_first_slice_probe_recon_v0.md   # N=1 evidence
  - forseti/product/spines/capture/core/source_families/social_media/tiktok/tiktok_logged_out_first_slice_probe_plan_v0.md
...
- **TikTok Shop session boundary.** Public logged-out Shop content remains `public_logged_out` when no account is used; this section's dedicated creator-account requirement does not relabel it `entitled_session`. The Shop sibling inherits the US-egress gate, public-content boundary, secret hygiene, and agent no-solve rule, while owning its separate operator challenge-handoff behavior.
- **TikTok Shop collection route.** After this shared access gate, brand-storefront/PDP collection follows `tiktok_shop_capture_lane_spec_v0.md`. That sibling owns native store-card navigation, dwell and microbatch behavior, operator challenge handoff, offer and product-linked creator-shelf fields, claim limits, and Data Lake admission; do not import those Shop-specific rules into creator onboarding.
