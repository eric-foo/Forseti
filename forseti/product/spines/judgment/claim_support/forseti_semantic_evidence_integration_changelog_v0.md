# Semantic Evidence Integration Changelog

```yaml
retrieval_header_version: 1
artifact_role: Version history and compatibility provenance
scope: Dated semantic integration changes, corrections, and replay exceptions.
use_when:
  - Resolving a named historical version or compatibility exception.
  - Tracing a dated decision or superseded implementation.
authority_boundary: retrieval_only
```

For current behavior, start with the
[current contract](forseti_semantic_evidence_integration_contract_v0.md).
Entries describe their dated revisions and stamped interfaces; later corrections
supersede earlier accounts. They do not make every historical route current.
The contract keeps live compatibility warnings and points to current procedure
owners. Open this file only for the relevant version or historical question.

History through v117 was moved without rewriting entries from the contract at
[the extraction baseline](https://github.com/eric-foo/forseti/commit/a2474c493cf82339573ca3583c82a7557a1d9444).
Future material version history belongs here; current requirements remain in the
contract or its linked owning source. This history creates no new review step.

## Changelog

- `v123` / 2026-09-09 — placed actionable instruction-reader usage at the
  Evidence Consolidation operating entry. Incomplete instruction reads remain
  explicit; reuse does not waive fresh checks. This documentation change does
  not change runtime interfaces, evidence admission, or consolidation methods.
