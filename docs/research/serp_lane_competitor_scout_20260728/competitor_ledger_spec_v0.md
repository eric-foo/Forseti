...
consume the sole automatic-validation attempt, and attaches the stored
receipt at seal time. Claiming before capture is the caller's ordering
obligation; the adapter records the claim but cannot observe capture.
Caller-supplied prior receipts are rejected.
...
launching a hot duplicate. That exclusivity is store-local: the claim id
derives only from the subject/entity pair, so the guarantee holds only while
every attempt shares one `--store-root`.
