...
| `forseti-harness/runners/` | CLI entry points; enumerate only on demand with `git ls-files forseti-harness/runners/*.py`. Subdirectories are NOT entry points and are missed by that glob — use `git ls-files forseti-harness/runners/` to see them. Current: `serp_fullbank_analysis/` (research derivations backing the SERP lane's findings ledger; operator-drive-pinned, not CI-runnable — see its README). |
