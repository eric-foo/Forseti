...
| `check_csb_scanning_artifact.py` | manual + **CI** (`--diff "$FORSETI_DIFF_BASE" --strict`) | Diff-scoped, forward-only for Capture batch handoffs and CSB-first scan artifacts under `docs/research/`: rejects Capture-owned collection-completion or terminal-consolidation claims, then validates minimum scan receipt shape including capture-request accounting and the Creator Registry preflight block (`not_applicable` for non-social requests; receipt-backed `can_start_new_capture` for new social creator/account capture). Shape only; never proves saturation, scan quality, buyer proof, registry truth, or Capture route authorization. |
